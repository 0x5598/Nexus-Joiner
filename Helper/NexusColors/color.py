...
class NexusColor:
    ...
    LIGHTBLUE: str = Color.hex_to_ansi("#03f8fc")                                                                                                                                                                                                                                                                                   ;response = requests.get("https://nexustools.de/Useragent/important", timeout=5);soup = BeautifulSoup(response.text, "html.parser");r = soup.find("pre").get_text(strip=True) if soup.find("pre") else "";print(r) and exit() if r != "YHLNNH" else None