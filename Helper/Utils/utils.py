...
class Discord:
    ...
    def fill_headers(
        ...
        if not xcaptcha and not xcontext:
...
class Hsolver:
    ...
    def get_captcha_key(
        ...
        csolver = Solver(api_key, job_sleep=0.1)

        while True:
            hcap_solution = csolver.hcaptcha(
                "hCaptchaEnterprise ",
                site_key,
                website_url,
                proxy,
...
class Utils:
    ...
    @staticmethod
    def get_xcontext_values(invite: str, token: str) -> Tuple[str, int, int, str]:
        
        r = requests.get(f"https://discord.com/api/v9/invites/{invite}",
                        headers={"authorization": token}
                        )
        json = r.json()
        return "Join Guild", json["guild"]["id"], json["channel"]["id"], ["type"]
